"""
Homework Set 4: Convolutional Neural Network for Image Classification
...
parser.add_argument(
    "-data", help="Designates dataset to use. Valid values: mnist, scenerec")
parser.add_argument(
    "-mode", help="Designates classifier to use. Valid values: nn, nn+svm")
parser.parse_args()
...
def load_data_scene(search_path, categories, size):
    ...
    for label_no in range(hp.scene_class_count):
        ...
        for i in range(size):
            ...
            im_vector = resize(im, (hp.img_size, hp.img_size)).reshape(
                1, hp.img_size * hp.img_size)
...
def format_data_scene_rec():
    ...
    train_images, train_labels = load_data_scene(
        train_path, categories, hp.num_train_per_category)
    test_images, test_labels = load_data_scene(
        test_path, categories, hp.num_test_per_category)
...
def format_data_mnist():
    ...
    with open("../data/train-images-idx3-ubyte.gz", 'rb') as f1, open("../data/train-labels-idx1-ubyte.gz", 'rb') as f2:
        ...
        train_images = np.frombuffer(
            buf1, dtype='uint8', offset=16).reshape(60000, 28 * 28)
    ...
    with open("../data/t10k-images-idx3-ubyte.gz", 'rb') as f1, open("../data/t10k-labels-idx1-ubyte.gz", 'rb') as f2:
        ...
        test_images = np.frombuffer(
            buf1, dtype='uint8', offset=16).reshape(10000, 28 * 28)
